select product.id, name, originprice, quantity, originalquantity
      ...
      content, product.option1, product.option2, product.option3, product.option4, product.option5,
...
-- 			user.updatedAt as user_updatedAt,
			...
			GREATEST(user.updatedat, product.updatedat) as updatedAt
      from product
      left join user on user.id=userId
      where unix_timestamp(product.updatedat) > :sql_last_value or unix_timestamp(user.updatedat) > :sql_last_value
